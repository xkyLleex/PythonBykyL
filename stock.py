...
def timecheck():
    settime=input("設定從現在到幾點ex(輸入09.06.01 or 23.16.21 or 00.00.00)24H制\n請輸入時間(按Enter上一步):")
    if settime=="":return 1
    if re.search(r"\d{2}.\d{2}.\d{2}",settime)!=None:
        (hour,mins,sec)=settime.split(".")
        try:
            if 24 > int(hour) >= 0 and 60 > int(mins) >= 0 and 60 > int(sec) >= 0:return settime
            else:return 3
        except:return 4
    else:return 2
    #1 空白 2 時間格式錯誤 3 時間數值錯誤 6 ok
